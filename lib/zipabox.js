			...
			if (err || !(res.statusCode >= 200 && res.statusCode < 400)) {
				...
				try{
					...
					if (result["dist-tags"].latest){						
						var fs = require("fs");
						var ZipaboxPackage = JSON.parse(fs.readFileSync("zipabox/package.json",'utf8'));
						...
						if (running_version.major < latest_version.major) {
						    console.log("A major new update is available!".yellow.bold);
						} else if (running_version.minor < latest_version.minor || running_version.patch < latest_version.patch) {
						    console.log("A new minor or patch update is available.".yellow);
						} /*else {
