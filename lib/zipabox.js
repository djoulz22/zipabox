...
function SetDatas_FN(value,ONSUCCESS_FN,ONERROR_FN){
	var req = {
...
function SetDeviceValue_FN(uuid,attribute_id,value,ONSUCCESS_FN,ONERROR_FN){
			...
			for (var uuid in this.json) {
		            ...
		            for (var attr in devicejson.attributes) {
		                ...
		                if (typeof (attribute.value) != "undefined") {
		                    ...
		                    retval += "\r\n\t" + (uuid.green + "(" + devicejson.name.bold + ")" + " - " + attrname + "(" + attr.bold + ")" + " = " + attribute.value.bold);
		                }
		            }
		        }				
	}	
